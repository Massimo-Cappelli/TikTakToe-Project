...
def updateStats(gameStats, player, x, y, gameTurn):
    ...
    The stats array register x and y posiotn for each move.
    ex:
    [(1:0), (2:1), (1:1),...]
...
def writeStatsFile(gameStats, gameTurn, player, gameStatus):
    '''
    Write the stats file.
    The stats fail register all game done in a csv, for each game is a new row.
    The format has always 9 columns - moves (x,y) - the last column is the gameStatus:
    If draw gameTurn, 0
    If win gameTurn, player (1 or 2)

    ex:
    [(1:0), (2:1), (1:1),...,(6,1)] player 1 win at turn 6 
...
def game(gameBoard, player, gaemTurn):
    ...
    while validMove:
        ...
        checkValidMove(move[0], move[1], gameBoard)
    
    gameBoard=updateGameBoard(move[0], move[1], gameBoard, player)
    updateStats(gameStats, player, move[0], move[1], gaemTurn)
    gameStatus = checkGameStatus(gameBoard, player)

    if gameStatsus[0]==True:
        writeStatsFile(gameStats, player, gameTurn, gameStatus[2])
